...
router.post("/stream", chatLimiter, async (req, res) => {
    try {
        ...
        // Bu loop hem ilk stream'i hem de (varsa) tool output sonrası ikinci stream'i yönetir.
            ...
            while (true) {
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        // Ancak en kolayı, herhangi bir event objesindeki id falan değil, run_id'yi bulmaktır.
                        // event: thread.run.created -> data.id
                        if (evt.object === "thread.run" && evt.id) {
                            runId = evt.id;
                        }

                        // 1) Delta Text (Kullanıcıya İlet)
                        if (evt.object === "thread.message.delta" && evt.delta?.content) {
                            for (const c of evt.delta.content) {
                                if (c.type === "text" && c.text?.value) {
                                    ...
                                    // Client'a aynen ilet
                                    if (!clientClosed) {
                                        // Aynen iletmek yerine, sadece text delta olarak basıyoruz
                                        // Client'ımız event yapısını bildiği için, raw event'i iletebiliriz
                                        // Ancak event wrapper'ı ile yazdırmak daha güvenli
                        ...
                        // 2) Tool Call Delta (Buffer'a at, kullanıcıya gizle)
        ...
        if (requiresAction && toolCallsBuffer.length > 0) {
            ...
            if (toolOutputs.length > 0 && runId) {
                ...
                if (submitResp.ok) {
                    // İkinci stream'i tüket (Client'a bu sefer text gidecek)
        ...
        // Finalize
        // Assistant yanıtını DB'ye kaydet
        // Not: assistantTextBuffer parça parça geldi, birleştirilmiş hali burada.
        if (assistantTextBuffer) {
            try {
                await logChatMessage({
                    ...
                    handoff: null, // Tool varsa yukarıda "system" rolüyle kaydettik, duplicate olmasın
                    visitorId,
                    sessionId,
                    source,
                    meta
